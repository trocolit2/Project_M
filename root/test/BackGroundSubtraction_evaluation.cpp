...
cv::Mat loadGTImages(std::string path, std::string prefix, uint index, uint sizeDigit) {
...
double getBackgroundSegmentationResult(std::vector<std::string> input, BackGroundSubtraction *backgroundPTR, std::vector<cv::Scalar> *vector) {
    ...
    while (videoTools.isNextFrame()) {
        cv::Mat currentFrame = videoTools.getFrame();
        foreground = bkSubtraction.applySubtraction(currentFrame);
        std::string currentFrameStr = videoTools.getFileInfo()[5];
        std::string pathLocal = videoTools.getFileInfo()[5];
        std::cout << "FRAME " << currentFrameStr << std::endl;
        cv::imshow("Frame_Original", currentFrame);
...
//        cv::imshow("CURRENT SEGMENTATION", foreground);
        if ((currentFrameInt % GROUND_THRUTH_INTERVAL) == 0) {
            ...
            std::cout << "SIZE GT IMAGE " << gtMat.size() << " CHANNEL " << gtMat.channels() << std::endl;
            std::cout << "SIZE FORE " << foreground.size() << " CHANNEL " << foreground.channels() << std::endl;
            cv::Scalar table = EvaluationsTools::evaluationMaps(gtMat, foreground, &out);
...
//            cv::imshow("OUT CHECK", out);
            TestTools::writeImageJpeg("IMG" + currentFrameStr + pathLocal + ".jpeg", out);
    ...
    return currentFrameInt * 1.0 / (double(clock() - begin_time) / CLOCKS_PER_SEC);
...
cv::Scalar evalutationROCArea(BackGroundSubtraction *backgroundPTR) {
    ...
    std::vector<cv::Point2f> points = EvaluationsTools::curveROC(scores, &area);
...
BOOST_AUTO_TEST_CASE(GroundThruthTableTestCase) {
    ...
    BOOST_CHECK_EQUAL(groundThruthTable[0] + groundThruthTable[1] + groundThruthTable[2] + groundThruthTable[3], inputMat.size().height * inputMat.size().width);
...
BOOST_AUTO_TEST_CASE(GroundThruthImageTestCase) {
    ...
    cv::Mat gtMat = cv::Mat::zeros(480, 640, CV_8UC1);
    cv::Mat inputMat = cv::Mat::zeros(480, 640, CV_8UC1);
    cv::Mat imageCheck = cv::imread(std::string(PATH_RELATIVE_ROOT_TESTBIN) + "resource/evaluationTest/GroundThruthImageTest.jpeg");
    cv::Mat out;
    ...
    std::vector<uint8_t> imageCheckvector = TestTools::convertmat2vector(imageCheck);
    std::vector<uint8_t> outvector = TestTools::convertmat2vector(out);

    BOOST_CHECK_EQUAL_COLLECTIONS(imageCheckvector.begin(), imageCheckvector.end(), outvector.begin(), outvector.end());
...
BOOST_AUTO_TEST_CASE(curveROC) {
    ...
    std::vector<double> vector = { 0.9, 0.8, 0.7, 0.6, 0.5, 0.4, 0.3, 0.2, 0.1, 0 };
    double area = 0;
...
//
//            BackGroundSubtraction bkSubtraction(
//            METHOD_ADAPTIVE_BACKGROUND_LEARNING, cv::Mat::zeros(cv::Size(100, 100), CV_8UC3), parans);
//
//            cv::Scalar scoreActual = evalutationROCArea(&bkSubtraction);
//
//            std::cout << " index " << count << " [score,th,alpha,fps] " << cv::Scalar(scoreActual[0], i, j, scoreActual[1]) << std::endl;
//
//            if (bestSets[0] < scoreActual[0]) {
//                bestSets = cv::Scalar(scoreActual[0], i, j, scoreActual[1]);
//                bestCount = count;
//            }
//            std::cout << "Bests Results of " << name << " index " << bestCount << " [score,th,alpha,fps] = " << bestSets << std::endl;
//            count++;
//        }
//    }
//}
...
//
//                    BackGroundSubtraction bkSubtraction(
//                    METHOD_MEDIAN_FILTER, cv::Mat::zeros(cv::Size(100, 100), CV_8UC3), parans, morph);
//
//                    cv::Scalar scoreActual = evalutationROCArea(&bkSubtraction);
//                    std::cout << " index " << count << "[score,fps] " << scoreActual[0] << " " << scoreActual[1] << "[th,alpha,dilate,erode] " << cv::Scalar(i, j, k, l) << std::endl;
//
//                    if (bestSets[0] < scoreActual[0]) {
...
//                    }
//                    std::cout << "Bests Results of " << name << " index " << bestCount << "[score,fps] " << bestSets[0] << " " << bestSets[1] << "\n" << "[th,alpha,dilate,erode] " << bestSetsConfig
//                            << std::endl;
//                    count++;
//                }
//            }
...
BOOST_AUTO_TEST_CASE(MANUAL_Evaluation) {
    ...
    std::vector<double> medianfilterVector = { 53, 0.5 };
    std::vector<double> morphVector = { 9, 9 };

//    METHOD_MEDIAN_FILTER OK PAPER
//    METHOD_MIXTURE_OF_GAUSSIAN_V1BGS ok PAPER
//    METHOD_ADAPTIVE_BACKGROUND_LEARNING OK PAPER
//    METHOD_MULTILAYER_BGS OK PAPER
//    METHOD_LB_ADAPTIVE_SOM OK PAPER
    BackGroundSubtraction bkSubtraction(METHOD_MEDIAN_FILTER, cv::Mat::zeros(cv::Size(100, 100), CV_8UC3), medianfilterVector);
    std::cout << "CHEGOU AQUI 1\n";
    cv::Scalar scoreActual = evalutationROCArea(&bkSubtraction);
    std::cout << "CHEGOU AQUI 2\n";
    std::cout << "[score,fps] " << cv::Scalar(scoreActual[0], scoreActual[1]) << std::endl;
}
