...
void backgroundFunctioGenericTestImageSequence(std::vector<std::string> input, std::vector<std::string> inputGT, BackGroundSubtraction *backgroundPTR) {
    ...
    if (writeImagesGT) {
        ...
        std::string regexTemp = RegexStringTools::fillSizeDigit(std::stoi(inputGT[2]), std::stoi(gtInfo[4]));
        pathWrite = gtInfo[1] + "/" + gtInfo[2];
        foregroundGT = cv::Mat::zeros(videoTools.getFrame().size(), CV_8UC3);
        TestTools::writeImageJpeg(pathWrite + regexTemp + gtInfo[3], foregroundGT);
        showImages = true;
    }
    ...
    while (videoTools.isNextFrame()) {
        ...
        if (writeImagesGT) {
            ...
            std::string regexTemp = RegexStringTools::fillSizeDigit(std::stoi(numberFrame), std::stoi(gtInfo[4]));
            if (!foreground.empty())
                TestTools::writeImageJpeg(pathWrite + regexTemp + gtInfo[3], foreground);
        } else {
...
BOOST_AUTO_TEST_CASE(AdaptiveBackgroundLearningPath1TestCase) {
    ...
    BackGroundSubtraction bkSubtraction(METHOD_ADAPTIVE_BACKGROUND_LEARNING, cv::Mat::zeros(cv::Size(100, 100), CV_8UC3), setttings);
...
BOOST_AUTO_TEST_CASE(MixtureOfGaussianV1BGSPath1TestCase) {
    ...
    BackGroundSubtraction bkSubtraction(METHOD_MIXTURE_OF_GAUSSIAN_V1BGS, cv::Mat::zeros(cv::Size(100, 100), CV_8UC3), setttings);
...
BOOST_AUTO_TEST_CASE(MixtureOfGaussianV1BGSWithMorphPath1TestCase) {
    ...
    BackGroundSubtraction bkSubtraction(METHOD_MIXTURE_OF_GAUSSIAN_V1BGS, cv::Mat::zeros(cv::Size(100, 100), CV_8UC3), setttings, morph);
...
BOOST_AUTO_TEST_CASE(LBAdaptiveSOMPath1TestCase) {
    ...
    BackGroundSubtraction bkSubtraction(METHOD_LB_ADAPTIVE_SOM, cv::Mat::zeros(cv::Size(100, 100), CV_8UC3));
...
BOOST_AUTO_TEST_CASE(MultiLayerBGSPath1TestCase) {
    ...
    BackGroundSubtraction bkSubtraction(METHOD_MULTILAYER_BGS, cv::Mat::zeros(cv::Size(100, 100), CV_8UC3));
...
BOOST_AUTO_TEST_CASE(MedianFilterPath1TestCase) {
    ...
    BackGroundSubtraction bkSubtraction(METHOD_MEDIAN_FILTER, cv::Mat::zeros(cv::Size(100, 100), CV_8UC3), setttings);
...
BOOST_AUTO_TEST_CASE(MorpDilateErodeTest) {
    ...
    cv::Mat gtInput = cv::imread(path + "GT1_BW1.jpeg", CV_LOAD_IMAGE_GRAYSCALE);
    gtInput = gtInput > 0;
