...
std::vector<cv::Scalar> groundsTruthPoints(std::vector<cv::Scalar> *inputs) {
	...
	groundsTruths.push_back(
			cv::Scalar(-0.083333333333333329, -1, 2.0833333333333335));

	//case6
	(*inputs).push_back(cv::Scalar(0, 4.0 / 3, 21.0 / 5, 11));
	groundsTruths.push_back(
			cv::Scalar(2.3015873015873014, -1, 1.3333333333333333));
...
BOOST_AUTO_TEST_CASE(calcNormLineEquationLine2DTestCase) {
	...
	for (uint i = 0; i < groundsTruths.size(); ++i) {
		groundsTruths[i];
		double norm = sqrt(
				groundsTruths[i][0] * groundsTruths[i][0]
						+ groundsTruths[i][1] * groundsTruths[i][1]);
		cv::Scalar temp(groundsTruths[i][0] / norm, groundsTruths[i][1] / norm,
				groundsTruths[i][2] / norm);
		cv::Scalar outLineEquation = LineTools::calcNormlizeLineEquation(
				groundsTruths[i]);
