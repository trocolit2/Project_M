...
FuzzyChoquetIntegral::FuzzyChoquetIntegral() :
		firstTime(true), frameNumber(0), showOutput(false), framesToLearn(10), alphaLearn(
				0.1), alphaUpdate(0.01), colorSpace(1), option(2), smooth(true), threshold(
				0.67) {
...
void FuzzyChoquetIntegral::process(const cv::Mat &img_input,
		cv::Mat &img_output, cv::Mat &img_bgmodel) {
...
//    std::cout << "FuzzyChoquetIntegral parameters:" << std::endl;
	...
	if (option == 2)
		std::cout << "Fuzzing by 2 color components + 1 texture component"
				<< std::endl;
...
//  }
	...
	if (frameNumber <= framesToLearn) {
		if (frameNumber == 0)
			std::cout
					<< "FuzzyChoquetIntegral initializing background model by adaptive learning..."
					<< std::endl;

		if (img_background_f3.empty())
			img_input_f3.copyTo(img_background_f3);
		else
			img_background_f3 = alphaLearn * img_input_f3
					+ (1 - alphaLearn) * img_background_f3;
...
//		std::cout << " DEBUG 2 entrou no else" << std::endl;
		cv::Mat img_input_f1;
		...
		IplImage* lbp_input_f1 = cvCreateImage(
				cvSize(input_f1->width, input_f1->height), IPL_DEPTH_32F, 1);
		cvFillImage(lbp_input_f1, 0.0);
		fu.LBP(input_f1, lbp_input_f1);

		IplImage* lbp_background_f1 = cvCreateImage(
				cvSize(background_f1->width, background_f1->height),
				IPL_DEPTH_32F, 1);
		cvFillImage(lbp_background_f1, 0.0);
		fu.LBP(background_f1, lbp_background_f1);

		IplImage* sim_texture_f1 = cvCreateImage(
				cvSize(input_f1->width, input_f1->height), IPL_DEPTH_32F, 1);
		fu.SimilarityDegreesImage(lbp_input_f1, lbp_background_f1,
				sim_texture_f1, 1, colorSpace);

		IplImage* sim_color_f3 = cvCreateImage(
				cvSize(input_f3->width, input_f3->height), IPL_DEPTH_32F, 3);
		fu.SimilarityDegreesImage(input_f3, background_f3, sim_color_f3, 3,
				colorSpace);

		float* measureG = (float*) malloc(3 * (sizeof(float)));
		IplImage* integral_choquet_f1 = cvCreateImage(
				cvSize(input_f1->width, input_f1->height), IPL_DEPTH_32F, 1);

		// 3 color components
		if (option == 1) {
			fu.FuzzyMeasureG(0.4f, 0.3f, 0.3f, measureG);
			fu.getFuzzyIntegralChoquet(sim_texture_f1, sim_color_f3, option,
					measureG, integral_choquet_f1);
		}
		...
		if (frameNumber == (framesToLearn + 1))
			std::cout
					<< "FuzzyChoquetIntegral updating background model by adaptive-selective learning..."
					<< std::endl;

		IplImage* updated_background_f3 = cvCreateImage(
				cvSize(input_f1->width, input_f1->height), IPL_DEPTH_32F, 3);
		cvFillImage(updated_background_f3, 0.0);
		fu.AdaptativeSelectiveBackgroundModelUpdate(input_f3, background_f3,
				updated_background_f3, integral_choquet_f1, threshold,
				alphaUpdate);
	...
	firstTime = false;
	frameNumber++;
//	std::cout << " DEBUG 3 FINAL DO PROCESS" << std::endl;
}

void FuzzyChoquetIntegral::saveConfig() {
	CvFileStorage* fs = cvOpenFileStorage("./config/FuzzyChoquetIntegral.xml",
			0, CV_STORAGE_WRITE);
