...
AdaptiveBackgroundLearning::AdaptiveBackgroundLearning() :
		firstTime(false), alpha(0.05), limit(-1), counter(0), minVal(0.0), maxVal(
				1.0), enableThreshold(true), threshold(15), showForeground(
				true), showBackground(true) {
}

AdaptiveBackgroundLearning::AdaptiveBackgroundLearning(uint threshold,
		double alpha, bool showImage) :
		firstTime(false), alpha(0.05), limit(-1), counter(0), minVal(0.0), maxVal(
				1.0), enableThreshold(true), threshold(15), showForeground(
				true), showBackground(true) {
	this->threshold = threshold;
	this->alpha = alpha;
	this->showBackground = this->showForeground = showImage;
}

AdaptiveBackgroundLearning::~AdaptiveBackgroundLearning() {
}

void AdaptiveBackgroundLearning::process(const cv::Mat &img_input,
...
//    saveConfig();
	...
	if ((limit > 0 && limit < counter) || limit == -1) {
		...
		img_background_f.convertTo(img_new_background, CV_8U,
	...
	img_diff_f.convertTo(img_foreground, CV_8U, 255.0 / (maxVal - minVal),
			-minVal);
	...
	if (enableThreshold)
		cv::threshold(img_foreground, img_foreground, threshold, 255,
				cv::THRESH_BINARY);
...
void AdaptiveBackgroundLearning::saveConfig() {
	CvFileStorage* fs = cvOpenFileStorage(
			"./config/AdaptiveBackgroundLearning.xml", 0, CV_STORAGE_WRITE);
...
void AdaptiveBackgroundLearning::loadConfig() {
	CvFileStorage* fs = cvOpenFileStorage(
			"./config/AdaptiveBackgroundLearning.xml", 0, CV_STORAGE_READ);
