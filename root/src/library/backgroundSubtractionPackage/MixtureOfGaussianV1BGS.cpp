...
MixtureOfGaussianV1BGS::MixtureOfGaussianV1BGS() :
		firstTime(true), alpha(0.05), enableThreshold(true), threshold(15), showOutput(
				true) {
	std::cout << "MixtureOfGaussianV1BGS()" << std::endl;
}

MixtureOfGaussianV1BGS::MixtureOfGaussianV1BGS(uint threshold, double alpha,
		bool showImage) :
		firstTime(true), alpha(0.05), enableThreshold(true), threshold(15), showOutput(
				true) {
	this->alpha = alpha;
	this->threshold = threshold;
	this->showOutput = showImage;
}
MixtureOfGaussianV1BGS::~MixtureOfGaussianV1BGS() {
	//std::cout << "~MixtureOfGaussianV1BGS()" << std::endl;
}

void MixtureOfGaussianV1BGS::process(const cv::Mat &img_input,
...
void MixtureOfGaussianV1BGS::saveConfig() {
	CvFileStorage* fs = cvOpenFileStorage("./config/MixtureOfGaussianV1BGS.xml",
			0, CV_STORAGE_WRITE);
